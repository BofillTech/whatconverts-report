...
var LEADGEN_SYSTEM = "You score inbound sales phone calls for small businesses. The rubric is the same for every industry; a short business description tells you what a good lead looks like for this client. Speech-to-text errors are common; read past misspellings. Respond with ONLY a single JSON object and no other text.";
...
function leadgenPrompt(transcript, leadId, accountName, note) {
  return "Business: \"" + accountName + "\"" + (note ? "\nWhat they do / what a good lead looks like: " + note : "") + "\n\n" +
    "TRANSCRIPT:\n" + transcript + "\n\nEND TRANSCRIPT.\n\n" +
    "Assess the caller on four signals:\n" +
    ...
    "- NOT_RELEVANT = spam, robocall, vendor/sales pitch to the business, wrong number, job seeker, or an existing customer with a service/billing question\n\n" +
...
async function classifyLeadgen(apiKey, t, accountName, note) {
  ...
  var leadId = String(t.lead_id);
  try {
    var text = await callClaude(apiKey, LEADGEN_SYSTEM, leadgenPrompt(transcript, leadId, accountName, note), 600);
...
async function classifyBatch(apiKey, transcripts, accountName, accountId) {
